...
."("
."id INT PRIMAEY KEY AUTO_INCREMENT,"
."na CHAR(32),"
."pa TEXT,"
."idd TEXT"
.");";
...
//パスワードの入力内容を確認し, IDを生成する. IDとパスワードをユーザー情報としてデータベースに保存する
//IDでuniqid
$idd = uniqid();
if (!empty($tname) && !empty($tpas)){
	$sql = $pdo ->prepare("INSERT INTO USERS(na, pa, idd) VALUES(:na,:pa,:idd);");
	$sql -> bindParam(':na',$tname, PDO::PARAM_STR);
	$sql -> bindParam(':pa', $tpas, PDO::PARAM_STR);
...
<!--登録フォームを用意する, 項目は名前とパスワード-->
<form action = "" method = "post">
<p>登録名前;<input type = "text" name = "tnamae"></p>
<p>登録パスワード;<input type = "text" name = "tpas"></p>
