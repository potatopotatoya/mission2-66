...
//パスワードに何かはいっていたら、それをあらたなパスワードに設定
if(empty($pas)){
	fwrite($fp,$tiring[0]."\n");
}else{
	fwrite($fp, $pas."\n");
...
//コメント編集の場合の変数newstr
$newstr = $vhh."|".$_POST["namae"]."|".$_POST["komento"]."|".$t;

//fileの内容を書き込む(パスワードが入っているとき、いないときでかえる。入っていなかったらコピーするだけ)
if($word == $tiring[0]){
	for ($b = 1; $b < count($tiring); ++$b){
		$c = explode("|", $tiring[$b]);
		if($c[0] == $hk){
			echo $hk."は削除されました<br/>\n";
		}elseif($c[0] == $hh){
...
$str = $newnumber."|".$_POST["namae"]."|".$_POST["komento"]."|".$t;

//ファイルに内容を書き込む
//ifで名前、コメントに内容がある時のみ書き込む
if (!empty($_POST["namae"]) && !empty($_POST["komento"]) && empty($vhh)){
	fwrite($fp, $str);
}
...
<!--削除・編集用のパスワード-->
<form action = "" method = "post">
<p> パスワード(編集・削除)；<input type = "text" name = "pas"></p>
<p> <input type = "submit" value = "パスワード"></p>
</form>
...
//新しくファイルを定義
$newf = file("kadai_2-6.txt", FILE_IGNORE_NEW_LINES);
